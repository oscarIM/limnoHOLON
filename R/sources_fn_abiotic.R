# ---- 1.  get_summ_stats — Estadística descriptiva por parámetro --------------
...
get_summ_stats <- function(data,
  ...
  # !! Parámetros granulométricos a excluir si matrix = "sedimento" !!
  pars_gran <- c(
  ...
  # --- 1.3 Filtrado condicional por matrix ------------------------------------
  # !! pars_no_LC: parámetros que NO tienen límite de cuantificación !!
  ...
  summ_pars <- data_plot %>%
    ...
    dplyr::mutate(
      # !! pars_no_LC → prop_bld = NA (no aplica LC) !!
      prop_bld = dplyr::case_when(
        col_pars %in% pars_no_LC ~ NA,
        .default = prop_bld
      ),
...
# ---- 2.  plot_parameter_levels — Barras de parámetros con normas -------------
#' @title Gráficos de barras de parámetros abióticos con normas
#' @description Variante de `plot_parameter_levels` que, cuando se entrega una
#'   tabla de normas (`ref_data`), filtra y dibuja solo aquellas referencias
#'   que se excedan.
#' @param data Tabla de datos de entrada.
...
plot_parameter_levels <- function(data,
  ...
  ## --- 2.2 Función auxiliar interna: fn_plot_aux -----------------------------
  fn_plot_aux <- function(data) {
    ...
    title_par <- ifelse(
      type_data == "Metales",
      "metálicos",
      stringr::str_to_lower(type_data)
    )
    ...
    # ·· 2.2.1 Función interna: make_plot ······································
    make_plot <- function(data_sub, idx = NULL) {
      # !! Dimensiones dinámicas según número de parámetros !!
      ...
      # !! Construcción condicional del ggplot según col_group y col_facet !!
      if (is.null(col_group) && is.null(col_facet)) {
        plot <- ggplot2::ggplot(
          ...
          ggh4x::facetted_pos_scales(
            y = list(label == "pH" ~ ggplot2::scale_y_continuous(limits = c(5, 9)))
          )
      } else if (is.null(col_group) && !is.null(col_facet)) {
        plot <- ggplot2::ggplot(
          data = data_sub,
          ggplot2::aes(x = factor(col_site), y = col_value, fill = type_par)
        ) +
          ggplot2::geom_bar(
            stat = "identity",
            position = ggplot2::position_dodge2(preserve = "single"),
        ...
        plot <- ggplot2::ggplot(
          data = data_sub,
          ggplot2::aes(x = factor(col_site), y = col_value, fill = col_group)
        ) +
          ggplot2::geom_bar(
            stat = "identity",
            position = ggplot2::position_dodge2(preserve = "single"),
      ...
      # ·· 2.2.3 Normas de referencia (hlines) — solo si ref_data != NULL ·······
      if (!is.null(ref_data)) {
        normas_tbl <- ref_data %>%
          dplyr::filter(.data$Sigla %in% unique(data_sub$col_pars))

        normas_tbl <- dplyr::left_join(
          data_sub, normas_tbl,
          by = c("col_pars" = "Sigla"),
        ...
        # !! Lógica de excedencia: pH tiene 2 límites (inferior / superior) !!
        df_excede <- df_long %>%
        ...
        # !! Solo se grafican normas que se exceden !!
        ...
        data_sub <- dplyr::left_join(
          data_sub, ref_data,
          by = c("col_pars" = "Sigla"),
          relationship = "many-to-many"
        )

        # !! Paleta fija de colores por norma !!
        colores_ref <- c(
          "#FF8C00", "#FF3030", "#912CEE", "#0072B2",
          "#009E73", "#000000", "#CC79A7", "gray50"
        )
        names(colores_ref) <- c(
          "Referencia norma ANZECC", "Referencia norma CCME",
          "Valor característico", "Referencia norma DS 144",
          "Referencia norma EPA", "Referencia norma NOAA",
          "Referencia norma NSCA Quintero", "Referencia norma UE 2023"
        )
        ...
        plot <- plot +
          ...
          ggplot2::guides(
            fill     = ggplot2::guide_legend(override.aes = list(linetype = NULL), order = 3),
            color    = ggplot2::guide_legend(override.aes = list(fill = NA), order = 1),
      ...
      # ·· 2.2.4 Guardado individual ············································
      data_source_clean <- stringr::str_replace(
        string = data_source, pattern = " ", replacement = "_"
      )
    ...
    # ·· 2.2.5 Paginación si hay >8 parámetros ·································
    if (n_pars > 8) {
      grupos_pars <- split(
  ...
  # !! Parámetros granulométricos excluidos !!
  pars_gran <- c(
    "LIM", "AMF", "AF", "AM", "AG", "AMG", "GRAN", "GUIJ",
    "Arena Fina", "Arena Gruesa", "Arena Media", "Arena Muy Fina",
    "Arena Muy Gruesa", "Fango", "Grava Fina", "Grava Muy Fina",
  ...
  # --- 2.4 Filtrado de parámetros con variabilidad > 0 ------------------------
  ...
  # !! Orden fijo de tipos de parámetros !!
  order_type <- c(
  ...
  # !! Colores especiales para estratos / réplicas !!
  if (!is.null(col_group)) {
  ...
  data_plot <- data_plot %>%
    dplyr::mutate(
      ...
      col_facet = if ("col_facet" %in% names(.)) {
        factor(col_facet, levels = ord_facet)
      } else {
        col_facet
      }
    ) %>%
  ...
  if (!is.null(col_group)) {
    data_plot <- data_plot %>%
      dplyr::mutate(col_group = factor(col_group, levels = unique(data_plot$col_group)))
  }

  # --- 2.7 Ejecución por tipo de parámetro ------------------------------------
  df_list <- data_plot %>%
...
# ----3.  plot_correlogram — Correlograma de Spearman --------------------------
...
plot_correlogram <- function(data,
  ...
  # --- 3.3 Filtrado de parámetros con variabilidad > 0 ------------------------
  ...
  # !! Advertencia si hay pocos sitios !!
  n_sitios <- nrow(data_corr)
  if (n_sitios <= 4) {
    warning(glue::glue(
      "¡Atención!, hay solo n = {n_sitios} puntos. Interpretar con extremo cuidado."
    ))
  }

  # --- 3.5 Matrices de correlación y p-valores --------------------------------
  coor_r <- rstatix::cor_mat(
    data_corr,
    method = "spearman",
  ...
  # --- 3.6 Preparación del data.frame para el plot ----------------------------
  # !! Solo triángulo inferior; solo muestra valor si p < 0.05 !!
...
#---- 4.  plot_pca — Biplot PCA con PERMANOVA opcional -------------------------
#' @title Biplot de PCA con PERMANOVA Opcional
#' @description Realiza un PCA y genera un biplot. Si se proporciona un factor,
#'   calcula PERMANOVA y añade elipses.
...
plot_pca <- function(data,
  ...
  # --- 4.2 Selección y renombrado dinámico ------------------------------------
  ...
  # --- 4.3 Filtrado de parámetros con variabilidad > 0 ------------------------
  selected_pars <- data_plot %>%
    dplyr::group_by(col_pars) %>%
    dplyr::summarise(
      prom    = abs(mean(col_value, na.rm = TRUE)),
  ...
  # --- 4.4 Paleta de colores por tipo de parámetro ----------------------------
  order_type <- c(
    "Fisicoquímicos", "Nutrientes", "Metales", "Orgánicos",
    "Bacteriológicos", "Bioquímicos", "TCLP inorgánicos"
  )
  ...
  # --- 4.5 PCA sin factor (biplot simple) -------------------------------------
  if (is.null(col_factor)) {
    ...
    # !! Agregar tipo de parámetro a las flechas !!
    ...
    plot <- ggplot2::ggplot() +
      ...
      ggplot2::geom_text(
        data = directions,
        mapping = ggplot2::aes(
          label = varname, x = xvar, y = yvar,
          angle = angle, hjust = hjust, color = type_par
        ),
        size = 3, family = "sans"
      ) +
      ggplot2::geom_text(
        ...
        nudge_y = 0.1, family = "sans", size = 3
      ) +
      ggplot2::scale_color_manual(
        name = "Tipo parámetro", values = cols_type, breaks = names(cols_type)
      ) +
      ggplot2::guides(
        color = ggplot2::guide_legend(title.position = "top", override.aes = list(label = " "))
      ) +
      ggplot2::geom_point(
        data = scores, mapping = ggplot2::aes(x = xvar, y = yvar), size = 1
      ) +
    ...
    # --- 4.6 PCA con factor (PERMANOVA + elipses) -------------------------------
  } else {
    to_pca <- data_pca %>%
    ...
    # !! PERMANOVA: test multivariado por factor !!
    test <- vegan::adonis2(
      to_pca[begin:ncol(to_pca)] ~ as.factor(to_pca$col_factor),
      method = dist
    )
    ...
    # !! Paleta dinámica según n de factores !!
    n_factor <- length(unique(scores$col_factor))
    col_pca <- if (n_factor <= 20) {
      ggsci::pal_d3("category20")(n_factor)
    } else {
      grDevices::colorRampPalette(ggsci::pal_d3("category20")(20))(n_factor)
    }
    ...
    subtitle <- glue::glue(
      "PERMANOVA por {stringr::str_to_lower(col_factor)}: R² = {R2}%, valor-p = {pval}"
    )

    plot <- ggplot2::ggplot(
      data    = scores,
      mapping = ggplot2::aes(x = xvar, y = yvar, fill = gr, group = gr)
    ) +
      ...
      ggplot2::scale_fill_manual(
        ...
        values = ggplot2::alpha(col_pca, 0.2),
        labels = names(col_pca)
      ) +
      ggplot2::geom_segment(
        data = directions,
        mapping = ggplot2::aes(x = 0, y = 0, xend = xvar, yend = yvar, color = type_par),
      ...
      ggplot2::geom_point(
        data = scores, mapping = ggplot2::aes(x = xvar, y = yvar),
        show.legend = FALSE
      ) +
      ggplot2::geom_text(
        data = scores,
        mapping = ggplot2::aes(x = xvar, y = yvar, label = col_site),
        nudge_y = 0.1, size = 2.5
      ) +
      ggplot2::scale_x_continuous(expand = c(0.1, 0.1)) +
      ggplot2::scale_y_continuous(expand = c(0.1, 0.1)) +
...
# ----5.  get_glm — Modelos GLM por grupos -------------------------------------
...
get_glm <- function(data,
  ...
  # !! Familia Gamma requiere valores > 0 !!
  if (family == "Gamma" && any(data_clean$col_value_internal <= 0, na.rm = TRUE)) {
    ...
    data_clean <- data_clean %>%
      dplyr::mutate(col_value_internal = ifelse(
        col_value_internal <= 0,
        col_value_internal + 1e-6,
        col_value_internal
      ))
  }

  # --- 5.3 Construcción de la fórmula -----------------------------------------
  ...
  # --- 5.4 Split por parámetro y ajuste --------------------------------------
  ...
  # --- 5.6 Procesamiento de resultados ----------------------------------------
  ...
  glm_table_processed <- glm_table %>%
    ...
    dplyr::mutate(significancia = dplyr::case_when(
  ...
  # !! %DEM: porcentaje de devianza explicada por cada efecto !!
...
#---- 6.  plot_boxplot — Boxplots con GLM externo ------------------------------
...
plot_boxplot <- function(data,
  ...
  # --- 6.2 Preparación de estadísticas GLM ------------------------------------
  stats_ready <- NULL
  ...
  if (add_test && !is.null(stats_df)) {
    if (!all(c("parametros", "pr_chi") %in% names(stats_df))) {
      stop("'stats_df' debe contener 'parametros' y 'pr_chi'.")
    }
  ...
  # --- 6.3 Función interna: build_plot ----------------------------------------
  build_plot <- function(d_plot, d_stats_glm) {
    ...
    p <- ggplot2::ggplot(d_plot, ggplot2::aes(x = col_x, y = col_y)) +
      ggplot2::geom_boxplot(
        na.rm = TRUE, outlier.colour = "black",
        outlier.shape = 19, outlier.alpha = 0.4,
        fill = "#E69F00"
      ) +
    ...
    # !! Test post-hoc Tukey HSD si 2 < n_factor <= 4 !!
    if (add_test) {
        ...
        if (nrow(dunn_res) > 0) {
          ...
          dunn_res <- dunn_res %>%
            rstatix::add_xy_position(
              x = "col_x", formula = col_y ~ col_x,
              data = d_plot, step.increase = 0.15
            ) %>%
          ...
          p <- p +
            ggpubr::stat_pvalue_manual(dunn_res, label = "p.adj.signif", hide.ns = TRUE, size = 3) +
            ggplot2::scale_y_continuous(expand = ggplot2::expansion(mult = c(0.05, 0.35))) +
            ggplot2::scale_x_discrete(expand = ggplot2::expansion(mult = c(0.2, 0.2)))
        }
      }

      # !! Etiqueta GLM valor-p en cada panel !!
  ...
  if (n_pars > 10) {
    ...
    for (i in seq_along(grupos)) {
      ...
      sub_stats <- if (!is.null(stats_ready)) {
        stats_ready %>% dplyr::filter(col_label %in% grupos[[i]])
      } else {
        NULL
      }
...
#---- 7.  plot_granulometria — Distribución granulométrica (stacked barplot) ---
...
plot_granulometria <- function(data,
  ...
  # !! Fracciones granulométricas estándar !!
  ...
  # --- 7.3 Filtrado de fracciones presentes -----------------------------------
  ...
  # --- 7.4 Factores: sitios y factor -----------------------------------------
  if (!is.null(ord_site)) {
    data_plot$col_site <- factor(
      data_plot$col_site,
      levels = intersect(ord_site, unique(data_plot$col_site))
    )
  } else {
  ...
  p <- ggplot2::ggplot(
    data_plot,
    ggplot2::aes(x = col_site, y = col_value, fill = col_pars)
  ) +
    ...
    ggplot2::guides(
      fill = ggplot2::guide_legend(title.position = "left", title = "Fracción granulométrica")
    ) +
    ggplot2::theme_bw(base_size = 10) +
    ggplot2::theme(
  ...
  # !! Facet condicional según col_factor y col_rep !!
  if (!is.null(col_factor)) {
    if (is.null(col_rep)) {
...
#--- 8.  line_plot_by_time — Series temporales con outliers (DBSCAN) -----------
...
line_plot_by_time <- function(data,
  ...
  ## ·· 8.1.1 Función auxiliar: find_eps_kneedle ································
  # !! Determina eps óptimo para DBSCAN via método del codo (kneedle) !!
  ...
  ## ·· 8.1.2 Función auxiliar: fn_get_ol ·······································
  # !! Detecta outliers con DBSCAN (minPts = 5, cluster == 0 → outlier) !!
  ...
  # --- 8.2 Selección y renombrado dinámico ------------------------------------
  ...
  # --- 8.3 Detección de outliers (DBSCAN por parámetro) -----------------------
  ...
  # --- 8.5 Construcción del gráfico -------------------------------------------
  if (tp == "clean") {
    # !! Modo limpio: excluye BLD y outliers !!
    data_viz <- data_plot %>%
      dplyr::filter(!is_BLD1, !ol_dbscan) %>%
      dplyr::mutate(
        group_aes = if ("col_group" %in% names(.)) {
          interaction(col_site, col_group, drop = TRUE)
        } else {
          col_site
        }
      )
    ...
    # !! Modo anotado: incluye outliers como puntos rojos + BLD con asteriscos !!
    matrix_safe <- stringr::str_replace_all(matrix, " ", "_")
    file_annot <- ensure_output_path(
      glue::glue("data_{matrix_safe}_PROMNA_annotated.csv"), output_dir
    )
    ...
    tryCatch(
      readr::write_csv(csv_out, file_annot),
      error = function(e) warning("No se pudo guardar CSV anotado")
    )
    ...
    p <- ggplot2::ggplot() +
      ggplot2::geom_point(
        data = data_viz %>%
          dplyr::filter(ol_dbscan, is.null(pars_annot) | !(col_pars %in% pars_annot)),
    ...
    # !! Asteriscos para parámetros anotados (pars_annot) !!
  ...
  # --- 8.6 Facets, escalas y tema ---------------------------------------------
  x_breaks <- if (is.numeric(data_plot$col_x)) {
    seq(
      min(data_plot$col_x, na.rm = TRUE),
      max(data_plot$col_x, na.rm = TRUE),
      length.out = 5
    ) %>%
      round() %>%
      unique()
...
#---- 9.  get_norm_exceedance — Excedencias de normas ambientales --------------
#' @title Calcular Excedencias de Normas Ambientales
#' @description Evalúa datos frente a una tabla de normas, calculando frecuencias
#'   y porcentajes de excedencia.
...
get_norm_exceedance <- function(data,
  ...
  if (!matrix %in% c("agua", "sedimento")) {
    stop("'matrix' debe ser 'agua' o 'sedimento'.")
  }

  # --- 9.2 Selección y renombrado --------------------------------------------
  ...
  # --- 9.3 Filtrado de parámetros con variabilidad > 0 ------------------------
  ...
  # --- 9.4 Validación y preparación de normas ---------------------------------
  if (!all(c("limite_1", "limite_2") %in% names(norm_data))) {
    stop("'norm_data' debe contener 'limite_1' y 'limite_2'.")
  }
  ...
  data_final <- dplyr::left_join(
    data_to_table, norm_prep,
    by = "col_pars_int",
    relationship = "many-to-many"
  )

  # --- 9.5 Pivoteo y cálculo de excedencias -----------------------------------
  df_long <- data_final %>%
    tidyr::pivot_longer(
  ...
  # !! Lógica de excedencia: pH es bidireccional (inferior + superior) !!
  df_excede <- df_long %>%
    dplyr::mutate(excede = dplyr::case_when(
  ...
  # --- 9.6 Tabla resumen de excedencias ---------------------------------------
  # !! Descripción del límite según norma y tipo !!
